...
def before_save(doc: "PayrollEntryCustom", method: str | None = None) -> None:
    ...
    if not frappe.flags.get(session_flag_key):
        frappe.msgprint(
            _(
                "FIXME: Разработчик! Реализуйте функцию `before_save` (расчет `total_payable`) для документа '{0}' типа PayrollEntryCustom. Текущее значение: {1}"
            ).format(
                doc.name or _("Новый документ"),
                doc.get("total_payable", _("не установлено")),
            ),
